			...
			return nodeComponent
				...
				: (
					context.index === 0
						...
						: (() => {
							const result = h('tbody', (context.parent as Table).children.slice(1).map((child) => h('tr', {
								align: (context.parent as Table).align?.[context.index] ?? 'left',
...
export function createVNodes(node: Parent, options: ToVNodeOptions = {}, context: Partial<CreateVNodeContext> = {}): VNode[] {
